...
(function () {
  ...
  const CLIENTS_DB_KEY = "rbr_clientes_db_v1";
  const CONTACTS_DB_KEY = "rbr_contatos_db_v1";
  ...
  let clienteTable = { sortField: "nome", sortDir: "asc", filterNome: "", filterEstado: "" };
  let contatoTable = { sortField: "nome", sortDir: "asc", filterNome: "" };
  let activeClientSubtab = "empresas";
  ...
  /* ===================== SEED DATA: CLIENTES / CONTATOS ===================== */
  function seedClients() {
    return [
      mkClient("Auto Peças Padre Cícero", "CE", "1975-04-15"),
      mkClient("Bezerra & Oliveira", "CE", "1962-01-31"),
      mkClient("Eletroparts", "CE", "2012-06-25"),
      mkClient("Ligpeça Atacado", "RN", "1941-10-14"),
      mkClient("Auri Auto Peças", "RN", "1991-09-17"),
      mkClient("Auto Giro Distribuidora", "PB", "1985-03-10"),
      mkClient("Ideal Distribuidora", "PB", "1992-10-05"),
      mkClient("Mega Giro Distribuidora", "PB", "2015-05-14"),
      mkClient("Peça Fácil Distribuidora", "PB", "2005-12-14"),
      mkClient("American Atacado", "PB", "2022-09-28"),
      mkClient("Fortgiro", "PB", "2006-08-31"),
      mkClient("Auto Norte Distribuidora", "PE", "1980-09-06"),
      mkClient("Ekko Parts Distribuidora", "PE", "1994-12-22"),
      mkClient("Mooven", "PE", "2023-07-20"),
      mkClient("Codisauto", "BA", "1992-07-02"),
      mkClient("Dismec", "BA", "2008-10-27"),
      mkClient("Gefpel", "BA", "1986-01-14"),
      mkClient("O Varejão Auto Peças", "BA", "1986-08-25"),
      mkClient("Roni Auto Peças", "BA", "1982-06-04"),
      mkClient("S. Silva Distribuidora", "BA", "1995-12-21"),
      mkClient("Vasco Imports", "BA", "1991-08-08"),
    ];
  }
  function mkClient(nome, estado, dataFundacao, obs) {
    return { id: uid(), nome, estado, dataFundacao, obs: obs || "" };
  }

  function seedContacts() {
    return [
      mkContact("Genilson", "Gerente", "25/07", "Auto Norte Distribuidora de Peças Ltda"),
      mkContact("Allan Porto", "Comprador", "19/07", "Auto Norte Distribuidora de Peças Ltda"),
      mkContact("Roberto Santiago", "Gerente", "28/07", "Eletroparts Dist. de Auto Peças Ltda"),
      mkContact("Severino Luiz", "Promotor", "03/07", "RBR Ricardo B Representações Ltda ME"),
      mkContact("Alexsander", "Vendedor", "15/07", "RBR Ricardo B Representações Ltda ME"),
      mkContact("Geraldo Magela", "Supervisor Administrativo", "06/07", "Pellegrino (PB)"),
      mkContact("Cristiane Lang", "Diretora Comercial", "07/07", "BZM (Power Parts/Di Fiera)"),
      mkContact("Augusto", "Supervisor de Vendas (PE/AL)", "11/07", "MP Lub (PE)"),
      mkContact("Renata", "Proprietária", "17/07", "Silveira Auto Peças"),
      mkContact("Allan Porto", "Gerente de Compras e Marketing", "19/07", "Auto Norte (PE)"),
      mkContact("Betânia", "Analista", "22/07", "Regis Auto Peças"),
      mkContact("Pergentino Neto", "Sócio", "28/07", "Estoque Fácil Distribuidora"),
      mkContact("Wilma Gadelha", "Comprador", "30/07", "Auto Peças Padre Cícero (Atacado - Filial 04 CE)"),
      mkContact("Adelson", "Proprietário", "21/07", "Delmar"),
    ];
  }
  function mkContact(nome, cargo, diaMes, empresa, obs) {
    return { id: uid(), nome, cargo, diaMes, empresa, obs: obs || "" };
  }
  ...
  function loadContactsDB() {
    try {
      const raw = localStorage.getItem(CONTACTS_DB_KEY);
      if (raw) { contacts = JSON.parse(raw); return; }
    } catch (e) { /* corrupted, fall through to seed */ }
    contacts = seedContacts();
    saveContactsDB();
  }
  function saveContactsDB() {
    localStorage.setItem(CONTACTS_DB_KEY, JSON.stringify(contacts));
  }
  function uid() {
  ...
  function switchView(view) {
    ...
    if (view === "clientes") switchClientSubtab(activeClientSubtab);
  ...
  function renderTable() {
    ...
    if (pageItems.length === 0) {
      tbody.innerHTML = `<tr><td colspan="11" style="text-align:center; padding:50px; color:var(--gray-500);"><i class="fa-solid fa-user-slash" style="font-size:26px; display:block; margin-bottom:10px;"></i>Nenhum colaborador encontrado.</td></tr>`;
    } else {
      tbody.innerHTML = pageItems.map(e => `
        <tr>
          ...
          <td>${e.dataAdmissao ? calcTenure(e.dataAdmissao) : "—"}</td>
  ...
  function copyCardText() {
    ...
    const { e, type, customText } = currentCardEmployee;
    let text;
    if (type === "generic") {
      text = customText;
  ...
  /* ===================== CLIENTES / PARCEIROS ===================== */
  function entityAvatarHTML(name, size) {
    const cls = size === "sm" ? "avatar-sm" : "avatar-lg";
    return `<div class="${cls}">${initials(name)}</div>`;
  }

  function switchClientSubtab(tab) {
    activeClientSubtab = tab;
    document.querySelectorAll(".subtab-btn").forEach(b => b.classList.toggle("active", b.dataset.subtab === tab));
    ["empresas", "contatos", "clientAniversariantes"].forEach(t => {
      document.getElementById("subtab-" + t).classList.toggle("hidden", t !== tab);
    });
    if (tab === "empresas") renderClientesTable();
    if (tab === "contatos") renderContatosTable();
    if (tab === "clientAniversariantes") renderClientBirthdays();
  }

  function populateClientFilterOptions() {
    const estados = [...new Set(clients.map(c => c.estado).filter(Boolean))].sort();
    fillSelect("filterClienteEstado", estados, "Estado");
  }

  function getFilteredSortedClients() {
    let list = clients.slice();
    if (clienteTable.filterNome) {
      const term = clienteTable.filterNome.toLowerCase();
      list = list.filter(c => c.nome.toLowerCase().includes(term));
    }
    if (clienteTable.filterEstado) list = list.filter(c => c.estado === clienteTable.filterEstado);
    list.sort((a, b) => {
      let va = (a[clienteTable.sortField] || "").toString().toLowerCase();
      let vb = (b[clienteTable.sortField] || "").toString().toLowerCase();
      if (va < vb) return clienteTable.sortDir === "asc" ? -1 : 1;
      if (va > vb) return clienteTable.sortDir === "asc" ? 1 : -1;
      return 0;
    });
    return list;
  ...
  function renderClientesTable() {
    ...
    const list = getFilteredSortedClients();
    const tbody = document.getElementById("clientesTableBody");
    if (list.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:50px; color:var(--gray-500);"><i class="fa-solid fa-building-circle-xmark" style="font-size:26px; display:block; margin-bottom:10px;"></i>Nenhuma empresa cadastrada.</td></tr>`;
      return;
    }
    tbody.innerHTML = list.map(c => {
      const years = calcCompanyYears(c.dataFundacao);
      return `
      <tr>
        <td><strong>${escapeHtml(c.nome)}</strong></td>
        <td>${escapeHtml(c.estado || "—")}</td>
        <td>${fmtDate(c.dataFundacao)}</td>
        <td>${c.dataFundacao ? years + (years === 1 ? " ano" : " anos") : "—"}</td>
        <td>
          <div class="row-actions">
            <button data-action="edit-cliente" data-id="${c.id}" title="Editar"><i class="fa-solid fa-pen"></i></button>
            <button data-action="delete-cliente" class="del" data-id="${c.id}" title="Excluir"><i class="fa-solid fa-trash"></i></button>
          </div>
        </td>
      </tr>`;
    }).join("");
  ...
  function getFilteredSortedContacts() {
    ...
    if (contatoTable.filterNome) {
      const term = contatoTable.filterNome.toLowerCase();
      list = list.filter(c => c.nome.toLowerCase().includes(term) || (c.empresa || "").toLowerCase().includes(term));
    }
    list.sort((a, b) => {
      let va = (a[contatoTable.sortField] || "").toString().toLowerCase();
      let vb = (b[contatoTable.sortField] || "").toString().toLowerCase();
      if (va < vb) return contatoTable.sortDir === "asc" ? -1 : 1;
      if (va > vb) return contatoTable.sortDir === "asc" ? 1 : -1;
      return 0;
    });
    return list;
  ...
  function renderContatosTable() {
    ...
    if (list.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:50px; color:var(--gray-500);"><i class="fa-solid fa-address-book" style="font-size:26px; display:block; margin-bottom:10px;"></i>Nenhum contato cadastrado.</td></tr>`;
      return;
    }
    tbody.innerHTML = list.map(c => `
      <tr>
        ...
        <td>${escapeHtml(c.diaMes || "—")}</td>
        <td>
          <div class="row-actions">
            <button data-action="edit-contato" data-id="${c.id}" title="Editar"><i class="fa-solid fa-pen"></i></button>
            <button data-action="delete-contato" class="del" data-id="${c.id}" title="Excluir"><i class="fa-solid fa-trash"></i></button>
          </div>
        </td>
      </tr>`).join("");
  }

  function renderClientBirthdays() {
    const now = new Date();
    const curMonth = now.getMonth();
    document.getElementById("clientAniversariantesSubtitle").textContent = `Aniversariantes de ${MESES_PT[curMonth]} — empresas e contatos`;

    const empresaItems = clients
      .filter(c => c.dataFundacao && new Date(c.dataFundacao + "T00:00:00").getMonth() === curMonth)
      .map(c => ({ kind: "empresa", day: new Date(c.dataFundacao + "T00:00:00").getDate(), data: c }));

    const contatoItems = contacts
      .filter(c => c.diaMes && parseInt(c.diaMes.split("/")[1], 10) - 1 === curMonth)
      .map(c => ({ kind: "contato", day: parseInt(c.diaMes.split("/")[0], 10), data: c }));

    const all = [...empresaItems, ...contatoItems].sort((a, b) => a.day - b.day);

    const grid = document.getElementById("clientBirthdayGrid");
    if (all.length === 0) {
      grid.innerHTML = `<div class="empty-state"><i class="fa-solid fa-handshake"></i><p>Nenhum aniversariante este mês.</p></div>`;
      return;
    }
    grid.innerHTML = all.map(item => {
      if (item.kind === "empresa") {
        ...
        return `
        <div class="person-card">
          ${entityAvatarHTML(c.nome)}
          <h4>${escapeHtml(c.nome)}</h4>
          <p>${escapeHtml(c.estado || "")}</p>
          <span class="date-chip blue-chip">${String(item.day).padStart(2, "0")} de ${MESES_PT[curMonth]} · ${years} ${years === 1 ? "ano" : "anos"}</span>
          <button class="btn btn-primary" data-action="client-card" data-kind="empresa" data-id="${c.id}"><i class="fa-solid fa-wand-magic-sparkles"></i> Gerar Card</button>
        </div>`;
      } else {
        const ct = item.data;
        return `
        <div class="person-card">
          ${entityAvatarHTML(ct.nome)}
          <h4>${escapeHtml(ct.nome)}</h4>
          <p>${escapeHtml(ct.cargo || "")}${ct.empresa ? " · " + escapeHtml(ct.empresa) : ""}</p>
          <span class="date-chip">${String(item.day).padStart(2, "0")} de ${MESES_PT[curMonth]}</span>
          <button class="btn btn-primary" data-action="client-card" data-kind="contato" data-id="${ct.id}"><i class="fa-solid fa-wand-magic-sparkles"></i> Gerar Card</button>
        </div>`;
      }
    }).join("");
  }
  ...
  function openClienteForm(id) {
    ...
    if (id) {
      ...
      if (!c) return;
      document.getElementById("clienteFormModalTitle").textContent = "Editar Empresa Parceira";
      document.getElementById("cliId").value = c.id;
      document.getElementById("cliNome").value = c.nome || "";
      document.getElementById("cliEstado").value = c.estado || "";
      document.getElementById("cliDataFundacao").value = c.dataFundacao || "";
      document.getElementById("cliObs").value = c.obs || "";
    } else {
      document.getElementById("clienteFormModalTitle").textContent = "Nova Empresa Parceira";
      document.getElementById("cliId").value = "";
    }
    openModal("clienteFormModalOverlay");
  }
  function handleClienteFormSubmit(ev) {
    ev.preventDefault();
    ...
    const data = {
      nome,
      estado: document.getElementById("cliEstado").value.trim().toUpperCase(),
      dataFundacao: document.getElementById("cliDataFundacao").value,
    ...
    saveClientsDB();
    closeModal("clienteFormModalOverlay");
    renderClientesTable();
  }
  function deleteCliente(id) {
    const c = clients.find(x => x.id === id);
    if (!c) return;
    askConfirm("Excluir empresa?", `Tem certeza que deseja excluir "${c.nome}"?`, () => {
  ...
  /* ---- CRUD Contato ---- */
  function openContatoForm(id) {
    const form = document.getElementById("contatoForm");
    form.reset();
    if (id) {
      const c = contacts.find(x => x.id === id);
      if (!c) return;
      document.getElementById("contatoFormModalTitle").textContent = "Editar Contato";
      document.getElementById("conId").value = c.id;
      document.getElementById("conNome").value = c.nome || "";
      document.getElementById("conCargo").value = c.cargo || "";
      document.getElementById("conDiaMes").value = c.diaMes || "";
      document.getElementById("conEmpresa").value = c.empresa || "";
      document.getElementById("conObs").value = c.obs || "";
    } else {
      document.getElementById("contatoFormModalTitle").textContent = "Novo Contato";
      document.getElementById("conId").value = "";
    }
    openModal("contatoFormModalOverlay");
  }
  function handleContatoFormSubmit(ev) {
    ev.preventDefault();
    const id = document.getElementById("conId").value;
    const nome = document.getElementById("conNome").value.trim();
    if (!nome) { toast("Informe o nome do contato.", "error"); return; }
    let diaMes = document.getElementById("conDiaMes").value.trim();
    if (diaMes && !/^\d{1,2}\/\d{1,2}$/.test(diaMes)) {
      toast("Use o formato DD/MM para o aniversário.", "error");
      return;
    }
    const data = {
      nome,
      cargo: document.getElementById("conCargo").value.trim(),
      diaMes,
      empresa: document.getElementById("conEmpresa").value.trim(),
    ...
    saveContactsDB();
    renderContatosTable();
    closeModal("contatoFormModalOverlay");
  }
  function deleteContato(id) {
    const c = contacts.find(x => x.id === id);
    if (!c) return;
    askConfirm("Excluir contato?", `Tem certeza que deseja excluir "${c.nome}"?`, () => {
  ...
  /* ---- Card generator for clients/contacts ---- */
  function openClientCard(kind, id) {
    let pseudo, title, sub, msg;
    if (kind === "empresa") {
      const c = clients.find(x => x.id === id);
      if (!c) return;
      const years = calcCompanyYears(c.dataFundacao);
      pseudo = { nome: c.nome, apelido: c.nome, foto: null };
      title = `${years} ${years === 1 ? "Ano" : "Anos"} de História!`;
      sub = escapeHtml(c.nome);
      msg = `Parabéns pelos ${years} ${years === 1 ? "ano" : "anos"}! A RBR Representações celebra essa parceria de sucesso. 🎉`;
    } else {
      const ct = contacts.find(x => x.id === id);
      if (!ct) return;
      pseudo = { nome: ct.nome, apelido: ct.nome, foto: null };
      title = "Feliz Aniversário!";
      sub = `${escapeHtml(ct.nome)}${ct.cargo ? " — " + escapeHtml(ct.cargo) : ""}`;
      msg = `${ct.empresa ? escapeHtml(ct.empresa) + ": a" : "A"} equipe RBR Representações deseja um dia repleto de alegrias! 🎉`;
    }
    currentCardEmployee = { e: pseudo, type: "generic", customText: msg };
    const preview = document.getElementById("cardPreview");
    preview.innerHTML = `
      ${avatarHTML(pseudo, "lg")}
      <h2>${title}</h2>
      <p>${sub}</p>
      <div class="card-msg">${msg}</div>
      <div class="card-logo">RBR Representações</div>
  ...
  function handleClientesCsvImport(ev) {
    ...
    reader.onload = e => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          clients.push(mkClient(
            obj["cliente"] || obj["nome"] || obj["razão social"] || "Sem nome",
            (obj["estado"] || "").toUpperCase(),
            normalizeDate(obj["data de aniversário da empresa"] || obj["data fundação"] || obj["datafundacao"] || "")
          ));
          imported++;
        }
        saveClientsDB();
        renderClientesTable();
        toast(`${imported} empresa(s) importada(s) com sucesso!`, "success");
      } catch (err) {
  ...
  function normalizeDate(str) {
    // aceita DD/MM/AAAA e converte para AAAA-MM-DD; já em ISO retorna direto
    if (!str) return "";
  ...
  function exportClientesCSV() {
    const headers = ["Cliente", "Estado", "Data de Aniversário da Empresa", "Idade"];
    const rows = clients.map(c => [csvEscape(c.nome), csvEscape(c.estado || ""), csvEscape(fmtDate(c.dataFundacao)), c.dataFundacao ? calcCompanyYears(c.dataFundacao) : ""]);
    const csv = [headers.join(";"), ...rows.map(r => r.join(";"))].join("\n");
    downloadBlob(new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" }), "clientes_parceiros_rbr.csv");
    toast("Exportação CSV concluída!", "success");
  }

  /* ---- Import/Export Contatos ---- */
  function handleContatosCsvImport(ev) {
    const file = ev.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = e => {
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const obj = {};
          headers.forEach((h, idx) => obj[h] = (row[idx] || "").trim());
          let diaMes = obj["dia/mês aniversário"] || obj["aniversário"] || obj["diames"] || "";
          const fullDateMatch = diaMes.match(/^(\d{1,2})\/(\d{1,2})\/\d{4}$/);
          if (fullDateMatch) diaMes = `${fullDateMatch[1].padStart(2, "0")}/${fullDateMatch[2].padStart(2, "0")}`;
          contacts.push(mkContact(
            obj["nome contato"] || obj["nome"] || "Sem nome",
            obj["cargo completo"] || obj["cargo"] || "",
            diaMes,
            obj["razão social"] || obj["empresa"] || ""
          ));
          imported++;
        }
        saveContactsDB();
        renderContatosTable();
        toast(`${imported} contato(s) importado(s) com sucesso!`, "success");
      } catch (err) {
        toast("Erro ao processar o arquivo CSV.", "error");
      }
      ev.target.value = "";
  ...
  function exportContatosCSV() {
    const headers = ["Nome Contato", "Cargo Completo", "Empresa", "Dia/Mês Aniversário"];
    const rows = contacts.map(c => [csvEscape(c.nome), csvEscape(c.cargo || ""), csvEscape(c.empresa || ""), csvEscape(c.diaMes || "")]);
    const csv = [headers.join(";"), ...rows.map(r => r.join(";"))].join("\n");
    downloadBlob(new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" }), "contatos_parceiros_rbr.csv");
    toast("Exportação CSV concluída!", "success");
  }
  ...
  function bindEvents() {
    ...
    // Clientes/Parceiros: subtabs
    document.querySelectorAll(".subtab-btn").forEach(btn => {
      btn.addEventListener("click", () => switchClientSubtab(btn.dataset.subtab));
    });

    // Clientes/Parceiros: empresas
    document.getElementById("btnNovoCliente").addEventListener("click", () => openClienteForm(null));
    document.getElementById("clienteFormModalClose").addEventListener("click", () => closeModal("clienteFormModalOverlay"));
    document.getElementById("clienteFormCancelBtn").addEventListener("click", () => closeModal("clienteFormModalOverlay"));
    document.getElementById("clienteForm").addEventListener("submit", handleClienteFormSubmit);
    document.getElementById("clientesTable").addEventListener("click", e => {
      const th = e.target.closest("th[data-sort]");
      if (th) {
        const field = th.dataset.sort;
        if (clienteTable.sortField === field) clienteTable.sortDir = clienteTable.sortDir === "asc" ? "desc" : "asc";
        else { clienteTable.sortField = field; clienteTable.sortDir = "asc"; }
        renderClientesTable();
        return;
      }
      ...
      if (btn.dataset.action === "edit-cliente") openClienteForm(id);
      if (btn.dataset.action === "delete-cliente") deleteCliente(id);
    });
    document.getElementById("filterClienteNome").addEventListener("input", e => { clienteTable.filterNome = e.target.value; renderClientesTable(); });
    document.getElementById("filterClienteEstado").addEventListener("change", e => { clienteTable.filterEstado = e.target.value; renderClientesTable(); });
    document.getElementById("importClientesCsvInput").addEventListener("change", handleClientesCsvImport);
    document.getElementById("btnExportClientesCsv").addEventListener("click", exportClientesCSV);

    // Clientes/Parceiros: contatos
    document.getElementById("btnNovoContato").addEventListener("click", () => openContatoForm(null));
    document.getElementById("contatoFormModalClose").addEventListener("click", () => closeModal("contatoFormModalOverlay"));
    document.getElementById("contatoFormCancelBtn").addEventListener("click", () => closeModal("contatoFormModalOverlay"));
    document.getElementById("contatoForm").addEventListener("submit", handleContatoFormSubmit);
    document.getElementById("contatosTable").addEventListener("click", e => {
      const th = e.target.closest("th[data-sort]");
      if (th) {
        const field = th.dataset.sort;
        if (contatoTable.sortField === field) contatoTable.sortDir = contatoTable.sortDir === "asc" ? "desc" : "asc";
        else { contatoTable.sortField = field; contatoTable.sortDir = "asc"; }
        renderContatosTable();
        return;
      }
      const btn = e.target.closest("button[data-action]");
      if (!btn) return;
      const id = btn.dataset.id;
      if (btn.dataset.action === "edit-contato") openContatoForm(id);
      if (btn.dataset.action === "delete-contato") deleteContato(id);
    });
    document.getElementById("filterContatoNome").addEventListener("input", e => { contatoTable.filterNome = e.target.value; renderContatosTable(); });
    document.getElementById("importContatosCsvInput").addEventListener("change", handleContatosCsvImport);
    document.getElementById("btnExportContatosCsv").addEventListener("click", exportContatosCSV);

    // Clientes/Parceiros: aniversariantes do mês (empresas + contatos)
    document.getElementById("clientBirthdayGrid").addEventListener("click", e => {
      const btn = e.target.closest("button[data-action='client-card']");
      if (btn) openClientCard(btn.dataset.kind, btn.dataset.id);
    });

    [["clienteFormModalOverlay"], ["contatoFormModalOverlay"]].forEach(([id]) => {
      document.getElementById(id).addEventListener("click", e => { if (e.target.id === id) closeModal(id); });
    });
    ...
    document.getElementById("btnClearDb").addEventListener("click", () => {
      askConfirm("Limpar base de dados?", "Todos os colaboradores, clientes e contatos cadastrados neste navegador serão removidos.", () => {
        employees = [];
        clients = [];
        contacts = [];
        saveDB();
    ...
    document.getElementById("btnSeedDb").addEventListener("click", () => {
      askConfirm("Restaurar dados de exemplo?", "A base atual será substituída pela equipe RBR e clientes/parceiros de exemplo.", () => {
        employees = seedData();
        clients = seedClients();
        contacts = seedContacts();
        saveDB();
    ...
    document.addEventListener("keydown", e => {
      if (e.key === "Escape") {
        ["formModalOverlay", "profileModalOverlay", "cardModalOverlay", "confirmModalOverlay", "clienteFormModalOverlay", "contatoFormModalOverlay"].forEach(id => closeModal(id));
      }
    });
  }
